# 🚀 Koe Syndicate Installation Guide

This comprehensive guide will walk you through setting up the Koe Syndicate AI-powered SDR system, from initial setup to full deployment.
...
# Create new project
gcloud projects create koe-syndicate-$(date +%s) --name="Koe Syndicate"
export PROJECT_ID=$(gcloud projects list --format="value(projectId)" --filter="name:Koe Syndicate" --limit=1)
...
```bash
# Create service account for Koe Syndicate
gcloud iam service-accounts create koe-syndicate-sa \
  --description="Koe Syndicate Service Account" \
  --display-name="Koe Syndicate"

# Grant necessary roles
gcloud projects add-iam-policy-binding $PROJECT_ID \
  --member="serviceAccount:koe-syndicate-sa@$PROJECT_ID.iam.gserviceaccount.com" \
...
gcloud iam service-accounts keys create koe-syndicate-key.json \
  --iam-account=koe-syndicate-sa@$PROJECT_ID.iam.gserviceaccount.com
```
...
gcloud alpha services api-keys create \
  --display-name="Koe Syndicate API Key" \
...
bq mk --dataset \
  --description="Koe Syndicate lead data" \
...
mv koe-syndicate-key.json .
chmod 600 koe-syndicate-key.json

# Or place in secrets directory
mkdir -p .secrets
mv koe-syndicate-key.json .secrets/
```
...
creds = service_account.Credentials.from_service_account_file('koe-syndicate-key.json')
...
docker build -f Dockerfile.ui_client -t koe-syndicate-ui-client .
docker build -f Dockerfile.lead_finder -t koe-syndicate-lead-finder .
docker build -f Dockerfile.lead_manager -t koe-syndicate-lead-manager .
docker build -f Dockerfile.sdr -t koe-syndicate-sdr .
docker build -f gmail_pubsub_listener/Dockerfile -t koe-syndicate-gmail-listener .

# Run with Docker Compose (if available)
docker-compose up -d

# Or run individual containers
docker run -d -p 8000:8000 --env-file .env koe-syndicate-ui-client
docker run -d -p 8081:8081 --env-file .env koe-syndicate-lead-finder
docker run -d -p 8082:8082 --env-file .env koe-syndicate-lead-manager
docker run -d -p 8084:8084 --env-file .env koe-syndicate-sdr
docker run -d --env-file .env koe-syndicate-gmail-listener
```
...
ls -la koe-syndicate-key.json
cat koe-syndicate-key.json | jq .type  # Should show "service_account"
```
