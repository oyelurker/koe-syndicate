...
The Gmail PubSub Listener operates as a bridge between Gmail and the Koe Syndicate system:
...
```bash
docker build -f gmail_pubsub_listener/Dockerfile -t koesyndicate-gmail-listener .
```
...
# Or using Docker
docker run koesyndicate-gmail-listener
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: gmail-listener
        image: koesyndicate-gmail-listener
        env:
        - name: PROJECT_ID
